...
단, 연봉의 순위는 다음과 같이 매긴다.

1. 연봉이 감소하는 방향으로 순위를 매긴다. 즉, 높은 연봉이 높은 순위를 가진다.
2. 같은 연봉인 경우 동일한 순위를 가진다.
...
`DENSE_RANK() OVER ()`을 이용하면 같은 연봉이면 공동 순위로 기록되고 번호의 끊김없이 연속적으로 매길 수 있다.  
