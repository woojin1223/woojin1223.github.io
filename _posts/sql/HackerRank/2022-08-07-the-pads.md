...
### 2. 각 직업별 빈도를 담은 문자열 (테이블2)

- 형식은 `There are a total of [occupation_count] [occupation]s.`
- [occupation_count]는 직업의 빈도, [occupation]은 직업명을 소문자로 나타낸 문자열
...
그리고 테이블1과 테이블2를 구분하기 위해 `table_type` 열을 만들어 1의 값을 넣는다.
...
그리고 테이블1과 테이블2를 구분하기 위해 `table_type` 열을 만들어 2의 값을 넣는다.
