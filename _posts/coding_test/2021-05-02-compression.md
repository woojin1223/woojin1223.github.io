...
1. 두 번째 글자 `A`는 사전에 있으나, 세 번째 글자까지인 `AK`는 사전에 없으므로, `A`의 색인 번호 1을 출력하고, `AK`를 사전에 28 번째로 등록한다.
1. 세 번째 글자에서 시작하는 `KA`가 사전에 있으므로, `KA`에 해당하는 색인 번호 27을 출력하고, 다음 글자 `O`를 포함한 `KAO`를 29 번째로 등록한다.
1. 마지막으로 처리되지 않은 글자 `O`에 해당하는 색인 번호 15를 출력한다.
...
def LZW(msg, word_dict):
    value = list(word_dict.values())[-1] + 1
    ...
    for i in range(1, len(msg) + 1):
        ...
        try:
            ...
            word_dict[next_word] = value
            msg = msg[i:]
            
            return msg, word_dict[word]

def solution(msg):
    word_dict = dict(zip(ascii_uppercase, range(1, len(ascii_uppercase) + 1)))
    result = []
    
    while msg:
        msg, output = LZW(msg, word_dict)
