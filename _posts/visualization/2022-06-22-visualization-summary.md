...
- 간단한 데이터라면 표로 정리할 수 있지만 데이터의 양이 많아지면 시각화는 필수
- 아래 그림의 왼쪽 표의 데이터는 그 자체로는 파악하기 힘들지만 꺾은 선 그래프로 시각화함으로 데이터의 추세를 쉽게 알 수 있음

![](/public/img/2022-06-22-visualization-summary/why_visualization1.PNG)

**2**. 데이터 분석에 대한 전문 지식이 없어도, 누구나 쉽게 데이터 인사이트를 찾을 수 있다.

- 인간은 매우 시각적인 동물
- 사람이 감각 기관을 통해 획득하는 정보의 80% 이상이 시각을 통해 얻어짐
- 데이터 시각화 차트를 활용하면 사람들의 기억에 보다 오래 남음

**3**. 요약 통계보다 정확한 데이터 분석 결과를 도출할 수 있다.

아래의 그림은 통계학자 앤스컴(Anscombe)이 시각화의 중요성을 보여주기 위해 만든 [앤스컴 콰르텟](https://ko.wikipedia.org/wiki/%EC%95%A4%EC%8A%A4%EC%BB%B4_%EC%BD%B0%EB%A5%B4%ED%85%9F)(Anscombe's quartet)이다. 

4개의 데이터셋은 요약 통계량이 6가지 부문에서 거의 동일하지만 시각화했을 때 전혀 다른 분포를 가진다. 

즉, 기술 통계량으로 데이터의 요약 수치를 찾는 것으로 끝나면 안되고 **시각화를 통해 데이터의 분포를 살펴 보는 과정**이 반드시 필요하다.

![](/public/img/2022-06-22-visualization-summary/why_visualization2.PNG)
